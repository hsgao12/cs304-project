...
// @route GET api/queries/avgFGhavingPPGOver23
// @desc Returns the name, average FG%, and PPG of players that have PPG>23 given a season
// @params JSON with season

router.get("/avgFGhavingPPGOver23", async(req, res) => {
    const {season} = req.body;
    const sql = `SELECT P.p_name, Avg(S.FG), S.PPG FROM Players P, Player_Has_Statistics S WHERE P.playerId = S.playerId AND S.season = '${season}' GROUP BY P.p_name HAVING S.PPG>23`;
    await executeSql(sql, req, res);
});
